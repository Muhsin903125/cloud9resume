...
const PortfolioIntegration = () => {
  ...
  const [newPortfolio, setNewPortfolio] = useState({ slug: '', password: '', isPrimary: false })
  ...
  if (loading) {
    return (
      <>
        <Head>
          <title>Portfolio - Cloud9 Resume</title>
        </Head>
        <div style={{ padding: '40px', textAlign: 'center' }}>Loading portfolio...</div>
      </>
    )
  }
  ...
  return (
    <>
      <Head>
        <title>Portfolio - {resume?.title} - Cloud9 Resume</title>
      </Head>
          ...
          {showPublishForm && (
            <div style={{ background: 'white', borderRadius: '8px', padding: '24px', marginBottom: '24px', border: `1px solid ${colors.border}` }}>
              ...
              <div style={{ display: 'grid', gap: '16px' }}>
                {/* Slug Input */}
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: colors.primary, marginBottom: '6px' }}>
                    Custom Slug (optional)
                  </label>
                  <input
                    type="text"
                    value={newPortfolio.slug}
                    onChange={(e) => setNewPortfolio({ ...newPortfolio, slug: e.target.value })}
                  ...
                  />
                  <p style={{ fontSize: '12px', color: colors.secondary, margin: '6px 0 0 0' }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewPortfolio({ ...newPortfolio, password: e.target.value })}
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                  <input
                    ...
                    onChange={(e) => setNewPortfolio({ ...newPortfolio, isPrimary: e.target.checked })}
              ...
              <div style={{ display: 'grid', gap: '12px' }}>
                {portfolios.map((portfolio) => (
                  ...
                  >
                      ...
                      <div>
                        <h3 style={{ fontSize: '16px', fontWeight: '700', color: colors.primary, margin: '0 0 4px 0' }}>
                          ...
                          {portfolio.is_primary && (
                            <span style={{ marginLeft: '8px', fontSize: '12px', background: '#fef3c7', color: '#d97706', padding: '2px 8px', borderRadius: '4px' }}>
                              PRIMARY
                            </span>
                          )}
                        </h3>
                        <p style={{ fontSize: '12px', color: colors.secondary, margin: 0 }}>
                          {portfolio.is_published ? 'Public' : 'Private'} {portfolio.password_hash ? '• Password Protected' : ''}
                        </p>
                      </div>
                      <button
                    ...
                    {/* Portfolio Link */}
                    <div style={{ padding: '12px', background: 'white', borderRadius: '4px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <input
                        type="text"
                        readOnly
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', fontSize: '12px' }}>
                      <div style={{ background: 'white', padding: '8px', borderRadius: '4px', textAlign: 'center' }}>
                        <div style={{ fontWeight: '700', color: colors.accent }}>{portfolio.view_count || 0}</div>
