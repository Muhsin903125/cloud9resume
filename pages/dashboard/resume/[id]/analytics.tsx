...
const AnalyticsDashboard = () => {
  ...
  const totalDownloads = portfolios.reduce(
    (sum, p) => sum + (p.download_count || 0),
    0
  );
  const totalClicks = portfolios.reduce(
  ...
  const avgAtsScore =
    atsHistory.length > 0
      ? Math.round(
          atsHistory.reduce((sum, a) => sum + a.score, 0) / atsHistory.length
        )
      : 0;
  ...
  if (loading) {
    return (
      <>
        <Head>
          <title>Analytics - Cloud9Profile</title>
        </Head>
  ...
  return (
    <>
      <Head>
        <title>Analytics - {resume?.title} - Cloud9Profile</title>
      </Head>
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: "32px" }}>
            ...
            >
              ...
              <StatCard
                label="Active Links"
                value={portfolios.filter((p) => p.is_published).length}
                icon="📱"
                color="#8b5cf6"
              />
            </div>
          ...
          <div style={{ marginBottom: "32px" }}>
            ...
            >
              <StatCard
                ...
                value={`${atsHistory[0]?.score || 0}%`}
                icon="📊"
                color="#ef4444"
              />
              <StatCard
                label="Average Score (30d)"
                value={`${avgAtsScore}%`}
                icon="📈"
                color="#3b82f6"
              />
              <StatCard
                ...
                value={`${atsHistory[0]?.keywords_matched || 0}/${
                  atsHistory[0]?.keywords_total || 0
                }`}
                icon="🔍"
                color="#10b981"
              />
              <StatCard
                ...
                value={
                  atsHistory[0] &&
                  atsHistory[1] &&
                  atsHistory[0].score > atsHistory[1].score
                    ? "↗️ Up"
                    : "↘️ Down"
                }
                icon="📉"
                ...
                <tbody>
                  {atsHistory.map((item, idx) => {
                    const matchRate = Math.round(
                      (item.keywords_matched / item.keywords_total) * 100
                    );
                    return (
                      <tr
                        key={idx}
