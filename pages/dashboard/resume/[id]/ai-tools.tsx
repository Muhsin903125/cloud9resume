...
const AIToolsUI = () => {
  ...
  const fetchResume = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/resumes/${id}`, { headers: { 'x-user-id': 'user-id-here' } })
  ...
  const useAIFeature = async (featureId: string) => {
    ...
    if (credits < feature.credits) {
      setError(`Insufficient credits. Need ${feature.credits}, have ${credits}`)
      return
    }
  ...
  const FeatureCard = ({ feature }: any) => {
    ...
    return (
      <div
        style={{
          background: 'white',
          border: activeFeature === feature.id ? `2px solid ${colors.accent}` : `1px solid ${colors.border}`,
          borderRadius: '8px',
          padding: '20px',
      ...
      >
        ...
        <p style={{ fontSize: '13px', color: colors.secondary, margin: '0 0 12px 0' }}>
          {feature.description}
        </p>
          ...
          <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: `1px solid ${colors.border}` }}>
            <button
              ...
              style={{
                width: '100%',
                padding: '8px 12px',
                background: hasCredits && canUse ? colors.accent : colors.border,
            ...
            >
              {processingFeature === feature.id ? 'Processing...' : hasCredits ? 'Use Feature' : 'Insufficient Credits'}
            </button>
          </div>
        )}
      </div>
  ...
  if (loading) {
    return (
      <>
        <Head>
          <title>AI Tools - Cloud9 Resume</title>
        </Head>
        <div style={{ padding: '40px', textAlign: 'center' }}>Loading AI tools...</div>
      </>
    )
  }
  ...
  return (
    <>
      <Head>
        <title>AI Tools - {resume?.title} - Cloud9 Resume</title>
      </Head>
      ...
      <div style={{ background: colors.light, minHeight: '100vh', padding: '24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          {/* Credits Status */}
          <div style={{ background: 'white', borderRadius: '8px', padding: '20px', marginBottom: '24px', border: `1px solid ${colors.border}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ fontSize: '14px', fontWeight: '600', color: colors.secondary, margin: '0 0 6px 0' }}>
                  Available Credits
                </h3>
                <p style={{ fontSize: '28px', fontWeight: '700', color: colors.accent, margin: 0 }}>
                  {credits}
                </p>
              </div>
              <div style={{ textAlign: 'right' }}>
                <p style={{ fontSize: '13px', color: colors.secondary, margin: '0 0 6px 0' }}>Current Plan</p>
                <p style={{ fontSize: '18px', fontWeight: '700', color: colors.primary, margin: 0, textTransform: 'capitalize' }}>
                  {userPlan}
                </p>
              </div>
              <button
          ...
          {/* Feature Tabs */}
          <div style={{ marginBottom: '24px', display: 'flex', gap: '8px', borderBottom: `2px solid ${colors.border}`, paddingBottom: '16px', overflowX: 'auto' }}>
            <button
              style={{
                padding: '8px 16px',
                background: activeFeature === null ? colors.accent : 'transparent',
            ...
            <button
              style={{
                padding: '8px 16px',
                background: activeFeature === 'writing' ? colors.accent : 'transparent',
            ...
            <button
              style={{
                padding: '8px 16px',
                background: activeFeature === 'optimization' ? colors.accent : 'transparent',
                color: activeFeature === 'optimization' ? 'white' : colors.primary,
                border: 'none',
                borderRadius: '6px',
                fontSize: '13px',
                fontWeight: '600',
          ...
          <div style={{ background: '#eff6ff', border: `1px solid #bfdbfe`, borderRadius: '8px', padding: '16px', marginBottom: '24px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: '700', color: colors.accent, margin: '0 0 8px 0' }}>
              💡 Pro Tips
            </h3>
            <ul style={{ fontSize: '13px', color: colors.secondary, margin: 0, paddingLeft: '20px' }}>
