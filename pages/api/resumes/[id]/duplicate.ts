...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (method === 'POST') {
      ...
      if (!sourceResume) return res.status(404).json({ error: 'Resume not found' })
      ...
      const planLimits: { [key: string]: number } = { free: 1, starter: 3, pro: 10, pro_plus: 100 }
      ...
      // Duplicate sections
      const sectionsToInsert = sourceResume.resume_sections.map((section: any) => ({
      ...
      if (userResumes && userResumes.length >= planLimits[userPlan]) {
        await supabase.from('credits_usage').insert({
          user_id: userId,
          action_type: 'duplicate_resume',
          resume_id: newResume.id,
          credits_amount: 1,
          metadata: { source_resume_id: id }
        })
      }
