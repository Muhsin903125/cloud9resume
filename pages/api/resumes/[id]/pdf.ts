...
function extractUserIdFromToken(req: NextApiRequest): string | null {
  try {
    ...
    const token = req.query.token as string || (authHeader && authHeader.startsWith('Bearer ') ? authHeader.substring(7) : null);
...
function transformToResumeData(resume: any, sections: any[]): any {
  ...
  const findSection = (type: string) => sortedSections.find(s => s.section_type === type)?.section_data || {};
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Transform Data
    const resumeData = transformToResumeData(resume, resume.resume_sections || []);
