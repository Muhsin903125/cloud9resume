...
function getTransporter() {
  ...
  if (
    ...
    console.warn(
      "Email configuration not set up. Set EMAIL_HOST, EMAIL_USER, and EMAIL_PASSWORD in .env"
    );
    return null;
  }
  ...
  try {
    ...
    if (!transport) {
      ...
      return res.status(200).json({
        success: true,
        message:
          "Email service not configured. Please set up email credentials in .env file.",
      });
    }
    ...
    await transport.sendMail({
      ...
      to: email,
      subject: "Your ATS Analysis Report - Cloud9Profile",
      html: htmlContent,
      ...
      <div class="container">
        ...
        <div class="keywords-list">
          <div class="keyword-group">
            <h3 style="color: #059669;">✓ Matched (${
              matchedKeywords.length
            })</h3>
            <div>
              ${matchedKeywords
                .slice(0, 10)
                .map((k: string) => `<span class="keyword-tag">${k}</span>`)
                .join("")}
              ${
          ...
          <div class="keyword-group">
            ...
            <div>
              ${missingKeywords
                .slice(0, 10)
                .map(
                  (k: string) => `<span class="keyword-tag missing">${k}</span>`
                )
                .join("")}
              ${
        ...
        <div style="text-align: center;">
          <a href="${
            process.env.NEXT_PUBLIC_APP_URL || "https://cloud9profile.com"
          }/dashboard/ats" class="cta-button">
            View Full Analysis
          </a>
        </div>

        <div class="footer">
          <p style="margin: 0;">Cloud9Profile - Professional Resume Builder</p>
