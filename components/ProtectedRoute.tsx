...
export function ProtectedRoute({ children, redirectTo = '/login' }: ProtectedRouteProps) {
  ...
  useEffect(() => {
    console.log('🛡️ ProtectedRoute: Checking authentication for', router.asPath)
    
    // Check if access token exists and is valid (not expired)
    const isValid = hasValidToken()

    console.log('🛡️ ProtectedRoute: Token validation result:', isValid)
    ...
    console.log('✅ ProtectedRoute: Valid token, allowing access')
