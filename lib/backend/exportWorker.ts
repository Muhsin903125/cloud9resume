...
export class ExportWorker {
    ...
    try {
      ...
      for (const job of pendingExports) {
        ...
        try {
          ...
          await this.updateJobStatus(job.id, 'failed', null, (err as Error).message)
        } finally {
  ...
  private async generatePDF(resume: any): Promise<Buffer> {
    ...
    ${resume.resume_sections.map((s: any) => `${s.section_type}: ${JSON.stringify(s.section_data)}`).join('\n\n')}
    `
   ...
   */
  private async uploadFile(fileContent: Buffer, fileName: string, resumeId: string): Promise<string> {
   ...
   */
  private async sendExportEmail(email: string, resumeTitle: string, fileUrl: string, format: string) {
    const mailOptions = {
      ...
      html: `
        ...
        <hr />
        <p style="color: #666; font-size: 12px;">© Cloud9 Resume - Professional Resume Builder</p>
      `
    }
   ...
   */
  private async updateJobStatus(jobId: string, status: 'completed' | 'failed', fileUrl: string | null, errorMessage?: string) {
